...
class HookBaseNode: SKSpriteNode, EventListnerNode {
	...
	func didMoveToScene() {
		...
		let ceilingFix = SKPhysicsJointFixed.joint(withBodyA: scene.physicsBody!,
		                                           bodyB: physicsBody!, anchor: CGPoint.zero)
	...
	func hookCat(catPhysicsBody: SKPhysicsBody) {
		...
		NotificationCenter.default.addObserver(self, selector: #selector(catTapped),
		                                       name: Notification.Name(CatNode.kCatTappedNotification),
		                                       object: nil)
	}
